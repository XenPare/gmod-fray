...
hook.Add("HUDPaint", "Fray KillFeed", function()
	...
	for key, data in SortedPairs(kf, true) do
		...
		draw.SimpleText(attacker, "fray_kf", ScrW() - 32 - 5 - victim_w - 5 - killed_w, _offset + 1, ColorAlpha(color_black, 220), TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
		draw.SimpleText(attacker, "fray_kf", ScrW() - 32 - 5 - victim_w - 5 - killed_w, _offset, color_attacker, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
	
		draw.SimpleText(killed, "fray_kf", ScrW() - 32 - 5 - victim_w, _offset + 1, ColorAlpha(color_black, 220), TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
		draw.SimpleText(killed, "fray_kf", ScrW() - 32 - 5 - victim_w, _offset, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)

		draw.SimpleText(victim, "fray_kf", ScrW() - 32, _offset + 1, ColorAlpha(color_black, 220), TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
